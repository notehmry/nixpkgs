...
let
  ...
  binYggdrasil = getExe cfg.package;
  binHjson = getExe pkgs.hjson-go;
  binJq = getExe pkgs.jq;

  generateKeysScript = writeExeclineScript "yggdrasil-genkeys.el" "" ''
    if -n { eltest -e ${keysPath} }
    if { mkdir --mode=700 -p ${builtins.dirOf keysPath} }
    pipeline -r {
      redirfd -w 1 ${keysPath}
      ${binJq} "to_entries|map(select(.key|endswith(\"Key\")))|from_entries"
    }
  ...
  configScript = writeExeclineScript "yggdrasil-config.el" "-s0" ''
    # Prepare config file.
    if { mkdir --mode=700 -p /run/yggdrasil }
    if {
      pipeline -r {

    ${optionalString settingsProvided ''
      if { echo ${builtins.toJSON (builtins.toJSON cfg.settings)} }
    ''}${
      optionalString configFileProvided (
        if config.systemd.enable then
          ''
            if {
              importas -S CREDENTIALS_DIRECTORY
              ${binHjson} -c "''${CREDENTIALS_DIRECTORY}/yggdrasil.conf"
            }
          ''
        else
          "if { ${binHjson} -c \"${cfg.configFile}\" }"
      )
    }${optionalString cfg.persistentKeys ''
      if { cat ${keysPath} }
    ''}${
      optionalString (!(settingsProvided || configFileProvided || cfg.persistentKeys)) ''
        if { ${binYggdrasil} -genconf -json }
      ''
    }
        exit
      }

      redirfd -w 1 /run/yggdrasil/yggdrasil.conf
      ${binJq} --slurp add
    }
